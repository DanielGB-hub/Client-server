...
if __name__ == "__main__":
    ...
    server_tcp.settimeout(0.2)
    server_tcp.listen(MAX_CONNECTIONS)

    server_logger.info(f"Запущен сервер, порт для подключений: {listen_addr}, "
                       f"адрес с которого принимаются подключения: {listen_port}. "
                       f"Если адрес не указан, принимаются соединения с любых адресов.")

    print(f"Запущен сервер, порт для подключений: {listen_addr}, "
          f"адрес с которого принимаются подключения: {listen_port}.")
