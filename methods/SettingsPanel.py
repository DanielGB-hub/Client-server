...
class SettingPanel(QtWidgets.QWidget):
    def __init__(self, parent=None, signal=None):
        ...
        # Обработчики кнопок
        self.setting.pushButton_7.clicked.connect(lambda: self.close())  # кнопка "настройки"
        ...
        if os.path.exists(os.path.join("data", "config.json")):
            with open(os.path.join("data", "config.json")) as file:
                ...
                self.setting.lineEdit_5.setText(data['password'])  # добавляем поля для пароля
    ...
    def save_config(self):
        ...
        password = self.setting.lineEdit_5.text()  # добавляем пароль
        ...
        self.setting.lineEdit_5.setStyleSheet("border-radius: 7px;")
        ...
        if len(nick) >= 3 and len(nick) <= 20:
            if len(password) >= 6:
                    ...
                    if server_port.isdecimal() and int(server_port) <= 65535:
                        ...
                        if not os.path.exists(os.path.join("data", "config.json")):
                            with open(os.path.join("data", "config.json"), "w") as file:
                                ...
                                data = {"nick": nick, "password": password, "server_ip": server_ip, "server_port": server_port}  # добавляем пароль
                ...
                self.setting.lineEdit_5.setStyleSheet("border: 2px solid red; border-radius: 7px;")
                self.setting.lineEdit_5.setText("Слишком короткий пароль")
        else:
